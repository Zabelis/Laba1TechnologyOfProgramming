...
Motorcycle::Motorcycle(string brand, string model, int engineCapacity, int enginePower, string terrainType) {
    this->brand = brand;
    this->model = model;
...
string Motorcycle::getDetails() {
    return "Motorcycle " + brand + " " + model + " " + to_string(engineCapacity) + " " + to_string(enginePower) + " " +
           terrainType;
}